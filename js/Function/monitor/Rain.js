...
define(['Controller/closeAll', 'Function/numberFC/FloatSelector', 'Function/drawgeojson', 'Function/monitor/area_point'],
 function(closeAll, floatSelector, draw, draw_pointInfo) { 
    ...
    function init(){
        ...
        floatSelector.on('floatMapDiv4', 'click', function(){
            closeAll.closeLayer();
            $('#floatMapDiv4').show();
            getData();
        });
    ...
    function getData(){ 
        var elem = floatSelector.getValue('floatMapDiv4');
        ...
        XHW.C.http.get(XHW.C.http.tobaccoUrl, '/actually/getArea', param, function(json){
            ...
            $('#floatMapDiv4 .floatMapDivTime').html(pubtime+'时');
            let data = json.data;
            if (data) {
    ...
    function draw_area_station(data,ele) {
        if(!data)return;
        var source = draw_pointInfo.drawMark(data,'mm',ele);
        if(!source)return;
        if(!layer){
    ...
    function draw_area_yan(data,ele) {
        if(!data)return;
        var source = draw_pointInfo.drawMark_yanzhan(data,'mm',ele);
        if(!source)return;
        if(!layer_yanzhan){
